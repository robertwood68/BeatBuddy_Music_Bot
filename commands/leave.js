...
module.exports = {
    ...
    async execute(client, interaction) {
        try {
            ...
            // NEED TO ADD IF FOR CHECKING BOTS VOICE CHANNEL VS THE MEMBERS VOICE CHANNEL


            
            // if (!client.queue.get(`${interaction.guild.id}`) || typeof client.queue.get(`${interaction.guild.id}`) === 'undefined') { // if user not in a voice channel 
            //     // create embed to hold current song
            //     const responseEmbed = new EmbedBuilder()
            //         .setAuthor({name: "Error"})
            //         .setColor("#0099E1")
